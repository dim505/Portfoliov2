...
export default function webstore(props) {
  return (
    <div>
      ...
      <p>
        2) It is integrated with Stripe which is a 3rd party payment processor.
      ...
      4) It makes various API calls to my API controllers to update, delete and
      insert records into the database. a) In terms of the back end, it keeps
      track of the items in the shopping cart by having a Shopping Cart header
      table to keep track of information regarding the session. Each item is
      entered in as its own separate row in the Shopping Cart line items table.
      The session ID is stored in the browser. This is used to match the right
      user to the cart when the user checks out. It collects information such as
      the name, email, billing information, delivery information and a token
      summarizing the credit card information. The token information is sent to
      ...
      the item ordered.
      <p>
        5) In terms of authentication. Auth0 uses JSON web tokens for the
        underlying mechanism to create access tokens. If the user does not
        ...
        correct HTTP Post data. Once a user logs in, he is able to view all
        ...
        to that order. Also, the logged-in user can view his account details
        ...
        out, that information can be pre-populated at check out if desired. This
        information will be stored in 3 tables. One table includes the name,
        email and flag needed to determine if the information should be
        pre-populated at check out. The other table contains the billing
        information. The 3rd table contains the delivery information.
      </p>
      <Typography classes={{ root: "AboutSection" }} variant="h5" gutterBottom>
      ...
      <p>
        {" "}
        I have seperated my app into 2 folders. The front end folder will
        contain my React JS front end and the back end folder will contain my
